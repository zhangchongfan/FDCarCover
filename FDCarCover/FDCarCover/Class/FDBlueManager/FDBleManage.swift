...
class FDBleManage: NSObject {
    ...
    var connectPeripherModel: FDPeripherModel?
    
    //系统蓝牙状态改变
    var bleDidUpdateState: ((PhoneBleState) -> ())?
    
    //蓝牙连接状态改变
    var bleConnectState: ((ConnectState) -> ())?
    
    //扫描回调
    var scanResult: ((FDPeripherModel) -> ())?
    
    //接收到手环返回的数据回调
    var receivePeripheralDataUpdate: ((Data) -> ())?
    
    var connected = false //是否连接，只有连接成功并且发现指定的服务和特征才算连接
    
    var autoConnect = true //是否自动重连，默认为true
    
    private var connectTimer: Timer?
    
    //单例
    static let shareManager = FDBleManage.init()
    ...
    func connectPeripheral(peripheralModel: FDPeripherModel) {//开始连接设备
        ...
        if #available(iOS 10.0, *) {
            connectTimer = Timer(timeInterval: 10.0, repeats: false) { [unowned self](timer) in
    ...
    func disconnectPeripheral() {//断开连接，如果手动则不再自动重新连接
